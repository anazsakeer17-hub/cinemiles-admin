// api/admin-rewards-redemptions.js
...
export default async function handler(req, res) {
  try {
    ...
    const { userId, page = 1, pageSize = 25, dateFrom, dateTo } = req.query
    ...
    let query = adminSupabase
      .from('admin_user_rewards_redemptions')
      .select('*', { count: 'exact' })
      .eq('user_id', userId)
      .order('redeemed_at', { ascending: false })
      .range(from, to)

    if (dateFrom) query = query.gte('redeemed_at', `${dateFrom}T00:00:00.000Z`)
    if (dateTo) query = query.lte('redeemed_at', `${dateTo}T23:59:59.999Z`)
    ...
    console.error('Admin rewards redemptions error:', err)

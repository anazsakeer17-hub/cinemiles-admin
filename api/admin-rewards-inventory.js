// api/admin-rewards-inventory.js
...
export default async function handler(req, res) {
  try {
    ...
    const { userId, page = 1, pageSize = 25, status = 'all', dateFrom, dateTo } = req.query
    ...
    let query = adminSupabase
      .from('admin_user_rewards_inventory')
      .select('*', { count: 'exact' })
      .eq('user_id', userId)
    ...
    if (dateFrom) query = query.gte('issued_at', `${dateFrom}T00:00:00.000Z`)
    ...
    console.error('Admin rewards inventory error:', err)
